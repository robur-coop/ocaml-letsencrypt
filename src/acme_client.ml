...
let request ?ctx ?meth ?headers ?body uri =
  C.request ?ctx ?meth ?headers ?body uri >>= function
  | Ok value -> ok value
...
let get_nonce ?ctx uri =
  let* r, _ = request ?ctx ~meth:`HEAD uri in
  match r.C.status with
  | 200 -> extract_nonce r
  | _ -> error_msgf "Invalid response from HEAD request to %s" uri

let rec post ?ctx ?(with_kid = false) cli data uri =
  ...
  let* resp, body = request ?ctx ~meth:`POST ~body ~headers uri in
  let* () =
    extract_nonce resp >>= function
    | Ok nonce -> cli.next_nonce <- nonce; ok ()
    | Error _ ->
        Log.err (fun m -> m "Couldn't extract nonce");
        ok () in
  match resp.C.status with
  | 400 ->
      let* err = S.return (Error.decode body) in
      begin match err.Error.error with
      | `Bad_nonce -> post ?ctx cli data uri
      | _ -> ok (resp, body) end
  | _ -> ok (resp, body)

let create_account ?ctx ?email cli =
  let uri = cli.d.newAccount in
  let data =
    let open Jsont.Json in
    let data = match email with
      | Some email -> [ mem (name "contact") (list [ string ("mailto:" ^ email) ]) ]
      | None -> [] in
    (mem (name "termsOfServiceAgreed") (bool true)) :: data
    |> object' in
  let* resp, body = post ?ctx ~with_kid:true cli data uri in
  match resp.C.status with
  | 201 ->
    let* account = Account.decode body |> S.return in
    let* () =
      let err = msgf "Account does not have status valid" in
...
let get_account ?ctx cli uri =
  let* resp, body = post ?ctx cli Jsont.Json.(null ()) uri in
  match resp.C.status with
  | 200 ->
    let* _ = S.return (Account.decode body) in
    ok ()
  | _ -> error_msgf "getAccount"

let find_account_url ?ctx ?email ~nonce key directory =
  let uri = directory.Directory.newAccount in
  let body =
    let open Jsont.Json in
    [ mem (name "onlyReturnExisting") (bool true) ] |> object' in
  let cli =
    { next_nonce = nonce
    ; account_key = key
    ; d= directory
    ; account_url = String.empty } in
  let* resp, body = post ?ctx ~with_kid:false cli body uri in
  match resp.C.status with
  | 200 ->
  ...
  | status -> error_msgf "newAccount: unexpected status %d" status

let challenge_solved ?ctx cli url =
  let data = Jsont.Json.(object' []) in
...
let finalize ?ctx cli csr url =
  let data =
    ...
    [ mem (name "csr") (string csr_as_b64) ] |> object' in
  let* resp, body = post ?ctx cli data url in
  match resp.C.status with
  | 200 ->
...
let rec process_order ?ctx solver cli sleep csr order_url headers order =
  ...
  | Processing ->
    ...
    let retry = retry_after headers in
    Log.debug (fun m -> m "sleeping for %d seconds" retry);
    sleep retry >>= fun () ->
    let* headers, order = get_order ?ctx cli order_url in
    process_order ?ctx solver cli sleep csr order_url headers order
  | Valid ->
  ...
  let data =
    ...
    let ids =
      List.map (fun hostname ->
          [ mem (name "type") (string "dns")
          ; mem (name "value") (string hostname) ] |> object')
        hostnames in
    [ mem (name "identifiers") (list ids) ] |> object' in
  let* resp, body = post ?ctx cli data cli.d.Directory.newOrder in
  match resp.C.status with
  | 201 ->
    let* order = S.return (Order.decode body) in
    (* identifiers (should-be-verified to be the same set as the hostnames above?) *)
    let* order_uri = location resp in
    process_order ?ctx solver cli sleep csr order_uri resp.C.headers order
